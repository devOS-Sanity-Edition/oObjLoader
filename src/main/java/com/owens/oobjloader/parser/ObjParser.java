...
public class ObjParser {
	...
	/**
	 * @param builder The ObjBuilder to use for parsing.
	 * @param path The path to the .obj file. Can either be an absolute path, or a path relative to jar resources.
	...
	public ObjParser(ObjBuilder builder, String path) throws IOException {
		...
		if (fromJar) {
			objFileStream = getClass().getClassLoader().getResourceAsStream(path);
		} else {
			objFileStream = new FileInputStream(objFile);
		}
		if (objFileStream == null) throw new FileNotFoundException("File could not be found at [" + path + "]");
		
		parseObjFile(objFileStream);
		builder.doneParsingObj(filename);
	}
	
	private void parseObjFile(InputStream objFileStream) throws IOException {
		int lineCount = 0;
		String fileContents = new String(objFileStream.readAllBytes());
...
//        System.err.println("Parsed as "+parsedList);
	...
	/**
	 * @param mtlFilePath The file path of the .mtl file. Either an absolute path or a path relative to jar resources.
	 * @throws IOException If an error occurs during parsing.
	 */
	private void parseMtlFile(String mtlFilePath) throws IOException {
		int lineCount = 0;
		File mtlFile = new File(mtlFilePath);
		Reader reader;
		if (mtlFile.exists()) {
			reader = new FileReader(mtlFile);
		} else {
			InputStream stream = getClass().getClassLoader().getResourceAsStream(mtlFilePath);
			if (stream == null) throw new FileNotFoundException("mtl file could not be found at path [" + mtlFilePath + "]");
			String fileContents = new String(stream.readAllBytes());
	...
	private void processReflectivityTransmissivity(String fieldName, String line) {
		int type = ObjBuilder.MTL_KA;
		if (fieldName.equals(MTL_KD)) {
			type = ObjBuilder.MTL_KD;
		} else if (fieldName.equals(MTL_KS)) {
			type = ObjBuilder.MTL_KS;
